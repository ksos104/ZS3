...
with open(r'/mnt/server14_hard1/msson/datasets/zs3_datasets/VOCZERO/all_classnames.json', 'w') as f_out:
    all_categories_json = []
    for cat in categories:
        all_categories_json.append(cat["name"])
    json.dump(all_categories_json, f_out)

with open(r'/mnt/server14_hard1/msson/datasets/zs3_datasets/VOCZERO/seen_classnames.json', 'w') as f_out:
    seen_categories_json = []
    for cat in categories_seen:
        seen_categories_json.append(cat["name"])
    json.dump(seen_categories_json, f_out)

with open(r'/mnt/server14_hard1/msson/datasets/zs3_datasets/VOCZERO/unseen_classnames.json', 'w') as f_out:
